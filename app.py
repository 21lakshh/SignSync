...
def main():
    ...
    # point_history.append([100, 200])  # Frame 1
    # point_history.append([102, 205])  # Frame 2
    # point_history.append([105, 210])  # Frame 3
    # point_history.append([150, 300])  # Frame 15
    # Finger gesture history 
    finger_gesture_history = deque(maxlen=history_length) # keeping track of the last 16 frame's classified gestures 
    # [0, 1, 2, 2, 1, 3, 4, 4, 2, 3, 3, 3, 2, 2, 1, 0]  # (16 values)
    mode = 0
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None: 
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                # hand_landmarks: stores 21 keypoints of the detected hands 
                # handness: determines if the detected hand is left or right 
                ...
                if hand_sign_id != "Not applicable":
                    ...
                    if current_gesture != last_gesture:  # Check if the gesture has changed
                        ...
                        # Call the function to generate a message from gestures
                        # llm_response = generate_message_from_gestures(recognized_gestures)
                        # print("LLM Response:", llm_response)  # Print the response from LLM
...
def select_mode(key, mode):
    ...
    if 48 <= key <= 57:  # 0 ~ 9
        number = key - 48
    if key == 110:  # normal mode
        mode = 0
    if key == 107:  # k keypoint classification mode for training 
        mode = 1
    if key == 104:  # h point histroy classification mode for training 
        mode = 2
    return number, mode
